...
O Prisma 7.8 com o driver adapter `@prisma/adapter-pg` foi utilizado para aproveitar as capacidades mais modernas de gerenciamento de ciclo de vida de conexão.
...
#### 9. Angular com Standalone Components

O Angular 21 foi escolhido com a abordagem de standalone components, eliminando a necessidade de NgModules. Cada componente declara suas próprias dependências, tornando o código mais explícito, modular e fácil de manter.

#### 10. Lazy loading em todas as rotas

Todas as páginas são carregadas sob demanda via lazy loading. Isso reduz o bundle inicial e melhora o tempo de carregamento percebido pelo usuário, especialmente relevante conforme o número de páginas cresce.

#### 11. Signals para estado local

O estado dos componentes é gerenciado com Signals (`signal`, `computed`) em vez de RxJS para casos simples. Signals são mais intuitivos para estado síncrono, reduzem boilerplate e se integram nativamente com o sistema de detecção de mudanças do Angular.

RxJS é mantido apenas onde faz sentido: chamadas HTTP via `HttpClient` e operadores como `finalize` para garantir que o estado de loading seja sempre resetado, mesmo em caso de erro.

#### 12. Interceptor global de erros HTTP

Um `HttpInterceptorFn` centraliza o tratamento de erros de API. Ele normaliza tanto os erros de validação do `ValidationPipe` (campo `message`) quanto os erros de negócio do filtro de exceções (campo `error`), garantindo que o frontend sempre receba uma mensagem legível independente da origem do erro.

#### 13. Separação de responsabilidades no front-end

A estrutura segue uma organização por feature com separação clara entre:

- `pages/` — componentes de rota com orquestração de estado
- `components/` — componentes de UI reutilizáveis e sem lógica de negócio
...
| Driver adapter (@prisma/adapter-pg) | Ciclo de vida de conexão explícito, pooling futuro     | Introduz dependência adicional em comparação ao modelo tradicional do Prisma |
| Signals em vez de NgRx              | Menos boilerplate, mais simples para o escopo atual    | Não escala bem para estado global complexo compartilhado entre features      |
| Standalone components               | Mais explícito, sem NgModules, tree-shaking melhor     | Requer importar dependências em cada componente individualmente              |

---

## Considerações de Segurança

Mesmo não sendo requisito do desafio, algumas práticas foram adotadas:

- DTOs validados com class-validator
- ValidationPipe com whitelist e forbidNonWhitelisted
- Exception Filter para evitar exposição de detalhes internos
- Uso de enums para restringir estados válidos
- Integridade referencial via foreign keys no PostgreSQL

Em um ambiente de produção seriam adicionados:

- JWT Authentication
- RBAC (Role Based Access Control)
- Rate Limiting
- Helmet
- Auditoria por usuário autenticado

---

## Escopo e Priorização

Devido ao tempo limitado do teste técnico, o foco foi:
...
Funcionalidades como autenticação, notificações e integrações externas foram propositalmente deixadas para fases futuras.
...
- **Feedback visual de ações** — Adicionar notificações toast para confirmar ações como criação, atualização e exclusão de incidentes.

### Médio prazo

- **Arquitetura orientada a eventos** — Emitir eventos de domínio (ex: `IncidentCreated`, `IncidentStatusChanged`) via event bus (NestJS EventEmitter ou RabbitMQ) para desacoplar efeitos colaterais como notificações do serviço principal.
...
- **Gerenciamento de estado global** — Adotar NgRx ou um store baseado em Signals para compartilhar estado entre features conforme o frontend cresce.
...
- **Testes E2E** — Adicionar testes end-to-end com Cypress ou Playwright cobrindo os fluxos críticos do frontend.
...
O Sentinel foi desenvolvido com foco em simplicidade, organização e clareza arquitetural, priorizando boas práticas de desenvolvimento sem introduzir complexidade desnecessária para o escopo proposto.

As decisões adotadas buscam equilibrar produtividade, manutenibilidade e escalabilidade futura, permitindo que a aplicação evolua gradualmente para um cenário mais próximo de produção conforme novas necessidades surgirem.
